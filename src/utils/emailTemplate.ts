const LOGO_URL =
  "https://res.cloudinary.com/df8sdofnq/image/upload/v1778233190/logo_xpf4mu.svg";

export const emailTemplate = (body: string): string => `
...
const detailsBox = (rows: { label: string; value: string }[]): string => `
...
const badge = (text: string, color: string, bg: string): string =>
  `<span style="display:inline-block;padding:4px 14px;border-radius:20px;background:${bg};color:${color};font-size:13px;font-weight:600;">${text}</span>`;

export { detailsBox, divider, badge };
